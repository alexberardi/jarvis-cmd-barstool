...
Get Barstool package for Jarvis
...
- **get_barstool** — Voice command (IJarvisCommand) (`commands/get_barstool/command.py`)
...
- **Shared code**: If you add shared modules, name the directory `get_barstool_shared/` (not `shared/`, `lib/`, `helpers/` — those collide on sys.path after install)
...
storage = JarvisStorage("get_barstool")
